...
export const RoutineItem = ({
  ...
  return (
    <View style={styleSheet.item}>
      ...
      {/* <TouchableOpacity style={styleSheet.image_container} onPress={()=>{downloadDataPressed(routine)}}> */}
      <TouchableOpacity
        style={styleSheet.image_container}
        onPress={() => {
          deleteRoutine(routine.id, deletingRoutine, refreshRoutines);
        }}>
        <Image
          source={require('../../assets/trash.png')}
          style={styleSheet.download_image}
        />
      </TouchableOpacity>
      <TouchableOpacity
        style={styleSheet.image_container}
...
function showToast() {
  ToastAndroid.show(
    'Se requieren los permisos para descargar los datos',
    ToastAndroid.SHORT,
  );
}
...
const saveRoutineInDownloads = async (routine: Routine) => {
  ...
  await FileSystemHelper.writeFileInDocumentsFolder(
    routineAsString,
    jsonFileName,
  );
...
const saveAudioInDownloads = async (routine: Routine) => {
  ...
  await FileSystemHelper.writeFileInDocumentsFolderBase64(
    routine.audio![0].data,
    audioName,
  );
...
const downloadDataPressed = async (
  routine: Routine,
  downloadData: any,
  savingInDevice: any,
) => {
